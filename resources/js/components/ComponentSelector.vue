                ...
                <div id="layout-selector-overlay">
                    <div id="layout-selector" class="overflow-scroll shadow-lg">
                        <div class="flex mb-4 h-100-p">
                          <div class="menu-container w-1/5 bg-grad-sidebar h-100-p pt-8">
                                <h4 class="ml-8 mb-4 text-xs text-white-50% uppercase tracking-wide">Tags</h4>
                                <ul class="list-reset mb-8">
                                    <li class="leading-tight mb-4 ml-8 text-sm">
                                        <a @click="toggleLayoutsTags" class="text-justify text-white no-underline dim cursor-pointer">
                                            All
                                        </a>
                                    </li>
                                    <li v-for="tag in availableTags" class="leading-tight mb-4 ml-8 text-sm">
                                        <a :id="tag" @click="toggleLayoutsTags" class="text-justify text-white no-underline dim cursor-pointer">
                                            {{ tag }}
                                        </a>
                                    </li>
                                </ul>
                          </div>
                          <div class="components-container w-4/5 bg-gray-500 h-100-p p-3">
                            <div class="flex p-3">
                                <div class="w-4/5">
                                    <div class="relative z-50 w-full max-w-xs">
                                        <div class="relative">
                                            <div class="relative">
                                                ...
                                                <input type="search"
                                                        v-model="message"
                                                        v-on:input="handleComponentSearch(message)"
                              ...
                              <div class="flex flex-wrap">
                                <div v-for="layout in visibleLayouts" class="w-1/3 component-layout-column">
                                    <div class="component-layout-selector max-w-sm rounded overflow-hidden shadow-lg cursor-pointer m-3"
                                         @click="addGroup(layout)">
                                      <img class="w-full" :src="layout.image" :alt="layout.title">
                                      <div class="px-6 py-4">
                                        <div class="font-bold mb-2">{{ layout.title }}</div>
                                        <p class="text-grey-darker text-base">
                                          {{ layout.description }}
                                        </p>
                                      </div>
                                      <div class="px-6 py-4" v-if="layout.tags">
                                        <span v-for="tag in layout.tags"
                                                class="inline-block bg-grey-lighter rounded-full px-3 py-1 text-sm font-semibold text-grey-darker mr-2" :data-tag="tag">
                                            #{{ tag }}
                                        </span>
                                      </div>
                                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            handleComponentSearch (search) {
                ...
                if (search.length > 0) {
                    ...
                    layouts.forEach(function (layout, layout_index) {
                        ...
                        if (
                            layout.description.toUpperCase().indexOf(search) !== -1 ||
                            layout.name.toUpperCase().indexOf(search) !== -1 ||
                            layout.title.toUpperCase().indexOf(search) !== -1 ||
                            layout.tags.join(' ').toUpperCase().indexOf(search) !== -1
                            ) {
            ...
            toggleLayoutsTags(event) {
                ...
                for (let el of document.querySelectorAll('.component-layout-selector')) el.closest('.component-layout-column').style.display = 'none';

                if (clicked_tag) {
                    for (let el of document.querySelectorAll('[data-tag="'+ clicked_tag +'"]')) {
                        el.closest('.component-layout-column').style.display = 'block';
                    }
                } else {
                    for (let el of document.querySelectorAll('.component-layout-selector')) el.closest('.component-layout-column').style.display = 'block';
                }
            },
